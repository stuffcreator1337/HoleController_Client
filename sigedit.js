socket.on('sending_sigs', function(data){
	...
	if(d["user"] == activeCharTab){
		...
		openSysTable(d["id"],d["name"],d["color"],"",data[0].sigs);
	};
});
...
function switchtab(tab1, tab2){
	...
	//console.log("switchtabStart");
	var label = document.getElementsByTagName("label");
	console.log(label);
	label[tab1].id = "checkedyes";
	label[tab2].id = "checkedno";
	var form = document.getElementsByTagName("form");
	form[tab1].style.display = 'block';
	form[tab2].style.display = 'none';
	//console.log(form[tab1].style.display);
}
